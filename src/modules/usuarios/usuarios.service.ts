  ...
  async createUsuarios(params: CreateUsuariosDto): Promise<IResponse> {
    try {
      ...
      if (verifyPass) {
        ...
        const userExist = await this.dataBase.usuarios.findFirst({
          where: {
            OR: [{ userName: params.userName }, { email: params.email }],
          },
        });
        if (userExist) {
          return dataResponseError('El usuario ya existe');
        }
        ...
        const insertAuth = await this.dataBase.password.create({
          data: {
            usuarioId: insert.id,
            esMaster: 1,
            titulo: 'Password Master',
            url: 'N/A',
